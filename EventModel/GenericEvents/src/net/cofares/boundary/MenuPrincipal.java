...
public class MenuPrincipal<D> implements Runnable {
    ...
    Source<D> eventManager;

    /**
     * Injecter la dépendence : la source Le gestionnaire des événement
     *
     * @param s
     */
    ...
    public void run() {
        int choix;
        GEvent<Source<D>, D> ev = eventManager.genEvent("Aucune réponse connue");
        try {
            while ((choix = menu()) != 0) {
                switch (choix) {
                    case 1:
                        ev = eventManager.genEvent("Create");
                        break;
                    case 2:
                        ev = eventManager.genEvent("Update");
                        break;
                    case 3:
                        ev = eventManager.genEvent("Delete");
                        break;
                    case 4:
                        ev = eventManager.genEvent("Read");
                        break;
                }
                if (ev != null) {
